...
function Home() {
  ...
  const relawanFathul_user = (localStorage.getItem('relawanFathul_user'));
  ...
  useEffect(() => {
    async function fetchSanityData() {
      try {
        setIsLoading(true);
        const sanityData = await sanityClient.fetch(`*[_type == 'data-kunjungan-fathul']{
  ...
  useEffect(() => {
    const initializeMap = async () => {
      const data = serverData && serverData.data && serverData.data.length > 0
        ? serverData.data.filter(item => item.user.name === relawanFathul_user)
        : [];
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(workbook, `data-kunjungan-fathul-${volunteerName}.xlsx`);
  };

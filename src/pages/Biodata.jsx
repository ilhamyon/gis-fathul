...
function Biodata() {
  ...
  const relawanFathul_userData = JSON.parse(localStorage.getItem('relawanFathul_userData'));
  const relawanFathul_id = (localStorage.getItem('relawanFathul_id'));
  console.log('cek id user: ', relawanFathul_id)
  ...
  useEffect(() => {
    async function fetchSanityData() {
      try {
        ...
        // Filter the data array based on relawanFathul_id
        const filteredData = sanityData.filter(item => item._id === relawanFathul_id);
  ...
  const updateSanityUser = async (relawanFathul_userData) => {
    try {
      const response = await fetch(`https://ln9ujpru.api.sanity.io/v2021-03-25/data/mutate/production`, {
        ...
        body: JSON.stringify({
          mutations: [
            {
              patch: {
                id: relawanFathul_id, // The _id of the document to update
                set: {
                  umur: relawanFathul_userData.umur,
                  gender: relawanFathul_userData.gender,
                  alamat: relawanFathul_userData.alamat,
                  faskes: relawanFathul_userData.faskes,
                  tb: relawanFathul_userData.tb,
                  bb: relawanFathul_userData.bb,
                  keluhan: relawanFathul_userData.keluhan,
                  telepon: relawanFathul_userData.telepon,
                },
              },
            },
          ],
        ...
        <div className="lg:flex">
          ...
          <div className="lg:w-1/2 lg:py-0 py-10 flex items-center">
            <form className="w-full lg:px-28 px-4 items-center" onSubmit={handleSubmit}>
              <h2 className="font-bold text-[18px] lg:text-4xl mb-10 text-gray-900 uppercase">Biodata <span className="text-sm">({relawanFathul_userData[0]?.name})</span></h2>
              <Input
                type="text"
                name="umur"
                placeholder="Umur"

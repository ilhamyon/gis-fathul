...
function Login() {
  ...
  useEffect(() => {
    async function fetchSanityData() {
      try {
        const sanityData = await sanityClient.fetch(`*[_type == 'user-relawan-fathul']{
          _id,
          name,
          email,
          type,
  ...
  const handleLogin = (event) => {
    ...
    const user = serverData.data.find(
      (relawanFathul_userData) => relawanFathul_userData.email === username && relawanFathul_userData.password === password
    );
    ...
    if (user) {
      ...
      localStorage.setItem("relawanFathul_token", token);
      localStorage.setItem("relawanFathul_user", user.name);
      localStorage.setItem("relawanFathul_id", user._id);
      localStorage.setItem('relawanFathul_userData', JSON.stringify(filteredData));
